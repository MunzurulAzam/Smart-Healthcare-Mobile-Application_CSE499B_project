...
import com.nearbydoctor.utils.AlarmReceiver;
import com.nearbydoctor.utils.LanguageSelectore;
import com.nearbydoctor.utils.bookapointmentgetset;

import static com.nearbydoctor.doctorfinder.Aboutus.setTypefaces;
...
public class BookApoinment extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LanguageSelectore.setLanguageIfAR( BookApoinment.this );
...
//        header.setTypeface(TF_ralewayRegular);
        ...
        if (click.equals( "signin" )) {
            Picasso.with(getApplicationContext()).load( getString( R.string.link1 ) + "uploads/" + image ).into( img );
        } else if (click.equals( "fb" )) {
            Picasso.with(getApplicationContext()).load( "https://"+image ).into( img );
        } else {
            Picasso.with(getApplicationContext()).load( getString( R.string.link1 ) + "uploads/" + image ).into( img );
        }

        String fulldetail_id = iv.getStringExtra("doctor_id");
        Log.e( "Bookid", "onCreate: "+fulldetail_id  );
...
//        Log.d("FUllDETAIL_id123",fulldetail_id);
                    ...
                    if (timepick != null && !timepick.isEmpty()) {
                        if (call != null && !call.isEmpty()) {
                            ...
                            if (coment != null && !coment.isEmpty()) {
                                ...
                                Toast.makeText(BookApoinment.this, R.string.enter_decription, Toast.LENGTH_LONG).show();

                            }
                        } else {
                            Toast.makeText(BookApoinment.this, R.string.enter_contectnumber, Toast.LENGTH_LONG).show();

                        }

                    } else {
                        Toast.makeText(BookApoinment.this,R.string.enter_time, Toast.LENGTH_LONG).show();
                    }
                } else {
                    Toast.makeText(BookApoinment.this, R.string.enter_date, Toast.LENGTH_LONG).show();
                }
    ...
    private void sendEmail() {
        ...
        try {
            ...
            Toast.makeText(BookApoinment.this, R.string.no_email_client_exite, Toast.LENGTH_LONG).show();
