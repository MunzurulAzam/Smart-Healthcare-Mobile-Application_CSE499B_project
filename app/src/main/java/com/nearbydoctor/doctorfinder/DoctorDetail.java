...
import com.nearbydoctor.utils.ConnectionDetector;
import com.nearbydoctor.utils.CustomMarker;
import com.nearbydoctor.utils.GPSTracker;
import com.nearbydoctor.utils.LanguageSelectore;
import com.nearbydoctor.utils.sqliteHelper;

import static com.nearbydoctor.doctorfinder.Home.TF_opensansRegular;
import static com.nearbydoctor.doctorfinder.Home.TF_ralewayRegular;
import static com.nearbydoctor.doctorfinder.Home.TF_ralewaybold;
...
public class DoctorDetail extends FragmentActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LanguageSelectore.setLanguageIfAR(DoctorDetail.this);
        ...
        Log.e("Docterid", "onCreate: " + doctor_id);
    ...
    private void setLayout() {
        ...
        btn_fb.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                try {
                    urlToShare = Obj_detail.getString("email") + getString(R.string.fbtt1) + Obj_detail.getString("services") + getString(R.string.fbt2) + Obj_detail.getString("phone") + getString(R.string.fbt3) + Obj_detail.getString("address");
                    ...
                    if (ShareDialog.canShow(ShareLinkContent.class)) {
                        ShareLinkContent linkContent = new ShareLinkContent.Builder().setContentUrl(Uri.parse(Obj_detail.getString("facebook"))).setQuote(urlToShare).build();
...
//                   Toast.makeText(DoctorDetail.this, getString(R.string.later_txt), Toast.LENGTH_SHORT).show();
                    ...
                    e.printStackTrace();
                    if (ContextCompat.checkSelfPermission(DoctorDetail.this, Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                        ActivityCompat.requestPermissions(DoctorDetail.this, new String[]{Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.WRITE_EXTERNAL_STORAGE}, 1);
                    }
                }
            }
        });
...
//                    intent.putExtra(Intent.EXTRA_STREAM, bmpUri);
                    ...
                    try {
                        urlToSharetwitter = getString(R.string.email1) + Obj_detail.getString("name") + "\n " + "\n\n" + Obj_detail.getString("twiter");
                        originalMessageEscaped = String.format("https://twitter.com/intent/tweet?source=webclient&text=%s", URLEncoder.encode(String.valueOf(Html.fromHtml(urlToSharetwitter)), "UTF-8"));
            ...
            public void onClick(View view) {
                ...
                try {
                    ...
                    try {
                        ...
                        Toast.makeText(DoctorDetail.this, getString(R.string.whatsapperror), Toast.LENGTH_LONG)
                                .show();
                    }

                } catch (Exception e) {
                    e.printStackTrace();
//                    Toast.makeText(DoctorDetail.this, getString(R.string.permission), Toast.LENGTH_SHORT).show();
                    if (ContextCompat.checkSelfPermission(DoctorDetail.this, Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                        ActivityCompat.requestPermissions(DoctorDetail.this, new String[]{Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.WRITE_EXTERNAL_STORAGE}, 1);
                    }
                }
        ...
        btn_favorite1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                sqliteHelper sqliteHelper = new sqliteHelper(DoctorDetail.this);
...
//                db = myDbHelpel.getWritableDatabase();
                ...
                try {
                    ...
                    values.put("distance", Obj_detail.getString("distance"));
...
//                    db = myDbHelper.getWritableDatabase();
                    ...
                    cur = db1.rawQuery("Delete from favorite where uniqueid =" + Obj_detail.getString("id") + ";", null);
                    if (cur.getCount() != 0) {
                        if (cur.moveToFirst()) {
                            do {
...
//                    myDbHelper.close();
    ...
    private void afterMapReady(double latitude, double longitude) {
        ...
        MarkerOptions markerOption = new MarkerOptions().position(
                ...
                .title(Obj_detail.optString("name"));
        ...
        try {
            ...
            Picasso.with(getApplicationContext()).load(getString(R.string.link1) + "uploads/" + Obj_detail.getString("icon")).into(img_profile);
    ...
    private void sendEmail() {
        ...
        try {
            ...
            Toast.makeText(DoctorDetail.this, R.string.no_email_client_exite, Toast.LENGTH_LONG).show();
...
//            db = myDbHelper.getReadableDatabase();
            ...
            try {
                cur = db1.rawQuery("select * from favorite where uniqueid=" + Obj_detail.getString("id") + ";", null);
